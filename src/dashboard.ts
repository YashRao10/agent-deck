...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .timeline { position: relative; max-height: 620px; overflow-y: auto; padding-right: 0.35rem; }
  .timeline::-webkit-scrollbar { width: 8px; }
  .timeline::-webkit-scrollbar-track { background: transparent; }
  .timeline::-webkit-scrollbar-thumb { background: var(--panel-border); border-radius: 999px; }
  .timeline::-webkit-scrollbar-thumb:hover { background: var(--text-faint); }
...
<div class="grid">
  <div>
    ...
    <section class="card">
      ...
      <p class="empty" id="tasks-empty" hidden>No tasks tracked yet. Run <code class="mono">macd assign</code> or <code class="mono">macd queue</code>.</p>
    </section>
  </div>
  <section class="card">
...
// Pool-counting logic mirrored in src/dashboard-logic.ts's countPooledTasks,
// which has unit test coverage this inline copy can't get. Keep them in sync.
function renderStats(sessions, tasks, messages) {
  ...
  const pooled = tasks.filter((t) => t.status === "pending" && t.assignedTo === "unassigned").length;
  ...
  const cards = [
    ...
    },
    {
      label: "Active tasks",
      value: activeTasks,
      breakdown: pooled ? \`<span class="chip pending">\${pooled} in pool, unclaimed</span>\` : "",
    },
